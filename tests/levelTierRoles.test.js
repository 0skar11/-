...
import { LEVEL_TIERS, ensureLevelTierRoles, offsetToSitAbove, sortRolesBottomUp } from '../src/services/leveling/levelTierRoles.js';
...
describe('level roles above chaos', () => {
  ...
  test('creates every tier, stacks them in order right above chaos and saves them as rewards', async () => {
    const guild = mockGuild([
      ...
      { id: '100000000000000003', name: '🔰 Trial Moderator', position: 3 },
      { id: '100000000000000004', name: 'Bot', position: 4 },
    ]);
    ...
    assert.deepEqual(names, ['@everyone', 'media', 'chaos', ...LEVEL_TIERS.map((tier) => tier.name), '🔰 Trial Moderator', 'Bot']);
  ...
  test('puts a tier that was moved out of place back in order', async () => {
    const guild = mockGuild([
      { id: '100000000000000000', name: '@everyone', position: 0 },
      { id: CHAOS_ROLE_ID, name: 'chaos', position: 1 },
    ...
    await level50.setPosition(-level50.position + 1, { relative: true }); // dragged below chaos
    const summary = await ensureLevelTierRoles(client, guild);
    assert.equal(summary.moved >= 1, true);
    const names = sortRolesBottomUp(guild.roles.cache.values()).map((role) => role.name);
    assert.deepEqual(names, ['@everyone', 'chaos', ...LEVEL_TIERS.map((tier) => tier.name), 'Bot']);
  });
});
