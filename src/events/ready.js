...
import { tidyChatChannelNames } from "../services/chatChannelNamesService.js";
...
export default {
  ...
  async execute(client) {
    try {
      ...
      try {
        const result = await tidyChatChannelNames(client);
        startupLog(`Chat channel names: renamed ${result.renamed}, unchanged ${result.unchanged}, errors ${result.errors}`);
      } catch (error) {
        logger.error("Failed to tidy chat channel names:", error);
      }
