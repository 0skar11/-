...
export default {
  ...
  async execute(message) {
    ...
    if (adding && target.type === 'role') {
      ...
      if (!role || role.managed || role.id === message.guild.id) return;
    }
    ...
    await message.channel.send(`${adding ? '🛡️ تمت إضافة' : '✅ تمت إزالة'} ${target.type === 'role' ? `<@&${target.id}>` : `<@${target.id}>`} ${adding ? 'إلى' : 'من'} قائمة Trust.`).catch(() => {});
  },
};
