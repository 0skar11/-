...
const COLORS = { levelUp: 0x5865f2, milestone: 0xf1c40f, rank: 0x5865f2, top: 0xf1c40f };
const MEDALS = ['🥇', '🥈', '🥉'];
...
export function buildRankEmbed(member, { level, xp, totalXp, xpNeeded, position, rankedCount }) {
    return {
        color: COLORS.rank,
        author: { name: member.displayName || member.user?.username, icon_url: member.displayAvatarURL?.() },
        title: '📊 المستوى',
...
/** `top`: the 10 highest members (entries from getLeaderboard) plus the caller's own place. */
export function buildTopEmbed(guild, entries, { callerId, callerEntry } = {}) {
    ...
    return {
        ...
        footer: { text: 'الترتيب حسب إجمالي XP • rank لمستواك' },
    };
}
